pro bps_001
compile_opt idl2
; A. first 9 channels are UV-2 fwhm 0.42 nm
; B. last 3 channels are VIS fwhm 0.63 nm
; define functions:
tri=lambda(x:1.0-abs(x)> 0.)
ones=lambda(n:replicate(1,n))
wav_uv2=[312.34, 317.35, 318.2, 321.2, 322.0, 325.0, 331.06, 340.0, 354.0]
wav_vis=[354.0, 380.0, 388.0]
x=[-1:1:.05]
band_uv2=tri(x/0.42)
band_vis=tri(x/0.63)
